...
function createMissile(kind:Kind,delay=0,offset=0){const p=params(current),mesh=BABYLON.MeshBuilder.CreateCylinder("missile-"+nextMissile,{height:3.2,diameter:.72,tessellation:10},scene),target=targetPoint.clone();mesh.rotation.z=Math.PI/2;mesh.material=kind==="warhead"?hostileMat:decoyMat;mesh.position.copyFrom(launchOrigin);target.z+=offset;missiles.push({id:nextMissile++,kind,mesh,start:launchOrigin.clone(),target,launchAt:elapsed+delay,duration:p.duration*(1+Math.abs(offset)*.002),arc:p.arc,alive:true,detected:false,pdDwell:0,smokeClock:0});}
